...
class zLocket:
    def __init__(self, device_token: str, target_friend_uid: str, num_threads: int, note_target: str):
        ...
        while True:
            Write.Print("[+] Nhập API KEY (Nhấn Enter để thoát) > ", Colors.red_to_yellow, interval=0.01)
            self.API_KEY = input().strip()
            if not self.API_KEY:
                print(f"{Fore.RED}⚠️ Đã bỏ nhập API KEY, thoát {self.NAME_TOOL}...{Style.RESET_ALL}")
                os._exit(1)
            if self._check_api_key():
                break
            else:
                print(f"{Fore.RED}⚠️ {self.response_data.get('msg', '⚠️ API KEY không hợp lệ, vui lòng nhập lại.')}{Style.RESET_ALL}")
    ...
    def _check_api_key(self) -> bool:
        try:
            response = requests.get(
                f"{self.firebase_app_check_url}?key={self.API_KEY}",
            ...
            if response_data.get('errorCode') == 0:
                ...
                self.messages.append(f"❌ API KEY không hợp lệ: {response_data.get('msg', 'Lỗi không xác định')}")
                return False
        ...
        try:
            ...
            if response_data.get('errorCode') == 1 or any(value == '' for value in response_data.values()):
                ...
                while True:
                    ...
                    if answer != "yes":
                        print(f"{Fore.YELLOW}❌ Exited {self.NAME_TOOL}...{Style.RESET_ALL}")
                        os._exit(1)
            ...
            if response_data.get('errorCode') == 0 and 'token' in response_data:
                ...
                self._zlocket_input()
                return True
            error_msg = response_data.get('msg', 'API error')
    ...
    def _zlocket_input(self):
        while True:
            self.target_friend_uid = Write.Input(
                "[+] Nhập URL Locket > ", Colors.red_to_yellow, interval=0.005).strip()
            if not self.target_friend_uid:
                print(
                    f"{Fore.RED}⚠️ Không Bỏ Trống URL Locket.{Style.RESET_ALL}")
                continue
            if not self.target_friend_uid.startswith(("https://locket.camera/invites/", "https://locket.camera/links/", "https://locket.cam/")):
                print(
            ...
            if not self.target_friend_uid:
                print(
                    f"{Fore.RED}⚠️ Không tìm thấy Url Locket, hãy thử nhập lại đúng link.{Style.RESET_ALL}")
                continue
            break
        ...
        while True:
            try:
                self.num_threads = int(Write.Input(
                    "[+] Nhập Threads (1 - 10000) > ", Colors.red_to_yellow, interval=0.005).strip())
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠉⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
    ...
    os.system("title 💰 zLocket Bulk Friend Request Tool by @vianhlaa 💰")
...
"""
    ...
    def _convert_url(self, url: str) -> str:
        ...
        if url.startswith("https://locket.cam/"):
            try:
                resp = requests.get(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36'}, timeout=self.request_timeout)
                if resp.status_code == 200:
                    ...
                    if match:
                        ...
                        if enc_link:
                            ...
                            self.messages.append("❌ Không tìm thấy tham số 'link' trong URL chuyển hướng.")
                            return ''
                    else:
                        self.messages.append("❌ Không tìm thấy đoạn chuyển hướng window.location.href.")
                        return ''
                else:
                    self.messages.append(f"❌ Lỗi HTTP khi tải trang locket.cam: {resp.status_code}")
                    return ''
            except Exception as e:
                self.messages.append(f"❌ Lỗi khi xử lý link locket.cam: {e}")
                return ''
        payload = {
            'type': 'toLong',
        ...
        try:
            ...
            error_msg = response_data.get('message', str(response_data)) if 'message' in response_data else 'API từ chối phản hồi'
